...
- `octowrap-check` hook id in `.pre-commit-hooks.yaml`, which runs `octowrap --check` so consumers no longer need to pass `--check` through `args`.
...
- A prose sentence containing a code-shaped fragment mid-line (e.g. `# step = i + 1, prev_step = i. Slice the panel arrays accordingly so`) no longer splits its paragraph when the prior wrap left the sentence boundary away from the end of the line, where the sentence-final-period signal cannot see it. `_looks_like_prose()` now also treats a keyword-free run of four or more bare words (e.g. "Slice the panel arrays accordingly so") as a prose signal, since such a run is never valid Python outside a string literal. The check runs on the line's code-visible text: string-literal contents are stripped by a quote-tracking scanner (so words inside a quoted string in genuinely commented-out code cannot trigger it) and nested comments after a `#` are ignored. A single quote immediately preceded by a letter is treated as a possessive apostrophe rather than a string delimiter, unless the preceding word is a string prefix like `f` or `rb`, so `user's` and `users'` stay visible while `f'...'` is stripped (in valid Python, a quote directly following an identifier letter is only ever a string opener when that word is a prefix). Runs containing Python keywords do not count, keeping real code like `value_a if flag else value_b` detected as code. Like the existing single-line scanners, the new scanner has no cross-line state; a quote that actually closes a string opened on an earlier line strips to the end of the line, which errs toward keeping the line classified as code.
...
### Fixed
- Explicit non-Python file paths (e.g. `octowrap README.md`) and `--stdin-filename` values without a `.py` suffix were processed as if they were Python source. Because octowrap operates lexically on `#`-prefixed lines rather than parsing to an AST, it would silently produce plausible-looking but wrong output from a Markdown file instead of failing loudly the way AST-based formatters do. Explicit file paths are now filtered by `.py` suffix with a `Warning: ... is not a Python file, skipping` message, and `--stdin-filename` is rejected at startup if its suffix is not `.py`. Bare stdin (`octowrap -` with no `--stdin-filename`) still trusts the caller, matching black/ruff convention; the README documents that editor integrations and shell pipelines own the filtering in that mode. Explicit paths inside excluded directories continue to bypass exclude patterns, preserving the existing escape hatch.
- Pressing the up arrow key (and other escape sequences) in interactive mode could still trigger "accept all remaining paragraphs" despite the 0.6.0 fix. The Unix `_getch()` path read via `sys.stdin.read(1)`, a buffered TextIOWrapper that pre-fetched all three bytes of `\x1b[A` from the kernel pipe into Python's internal buffer on the first read; the subsequent `select`-based drain only saw the OS file descriptor, missed the buffered bytes, and the trailing `A` resurfaced on the next prompt as the accept-all action. `_getch()` now reads via `os.read()` on the raw stdin file descriptor so every byte stays at the OS layer where the drain logic can reach it. A regression test wires `_getch()` up to a real `os.pipe()` pre-loaded with `\x1b[A` to guard against a recurrence.
- The `[X/Y]` progress indicator under-counted after an `A` (accept-all) press. `block_current` was incremented only at the prompt where `A` was entered; subsequent paragraphs auto-accepted under accept-all skipped the bump entirely, so a file with N changed paragraphs where the user pressed `A` on the first prompt finished with `block_current=1` instead of N, and the indicator at the next file's first prompt under-reported total progress by N-1. The recompute at file entry now adds the per-`A` auto-accept count from a new `_state["a_extras"]` mapping (computed once via `count_changed_blocks` when `A` fires, cleared if the `A` is later popped via undo), so `[X/Y]` reflects every silently auto-accepted paragraph in addition to explicit prompts. Existing undo semantics are preserved: `u` after `A` still rewinds to A's prompt position rather than to the last auto-accepted paragraph.
