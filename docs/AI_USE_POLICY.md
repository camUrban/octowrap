...
* Contributors are responsible for having the right to license their contribution under octowrap's MIT license.
...
* Commits:
  ...
  * `Signed-off-by:` should not be used. octowrap has no DCO, so the trailer carries no meaning here. Tooling that emits it unconditionally, such as Dependabot, is exempt.
